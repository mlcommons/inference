...
class BackendDeploy(backend.Backend):
    ...
    def load(self):
        ...
        model = PointPillars(nclasses=self.detection_classes.nclasses, painted=True)
        checkpoint = torch.load(self.lidar_detector)
    ...
    def predict(self, inputs):
        ...
        format_results = {}
        batched_pts = inputs['batched_pts']
        #batched_images = data_dict['batched_images'][0]
        scores_from_cam = []
        for i in range(len(inputs['batched_images'][0])):
            segmentation_score = self.segmentor(inputs['batched_images'][0][i])[0]
            scores_from_cam.append(self.painter.get_score(segmentation_score))

        points = self.painter.augment_lidar_class_scores_both(scores_from_cam, batched_pts[0], inputs['batched_calib_info'][0])
        ...
        for j, result in enumerate(batch_results):
                ...
                calib_info = inputs['batched_calib_info'][j]
                image_info = inputs['batched_img_info'][j]
                idx = inputs['batched_img_info'][j]['image_idx']
