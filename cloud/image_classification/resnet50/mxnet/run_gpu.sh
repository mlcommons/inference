...
DATABASE_NAME=resnet50_mxnet_full_trace # the name of database to publish traces to
MODEL_NAME=ResNet50 # model name
MODEL_VERSION="1.3.0" # model version
...
docker run --runtime=nvidia --network host -t -v $HOME/.config/carml:/root carml/mxnet-agent:amd64-gpu-latest predict dataset \
    --verbose \
    --publish=true \
