...
def top_p_filter(
        results_with_scores: List[Tuple[Any, float]], p: float = 0.9) -> List[Any]:
    """
    ...
    for i, ((result, score), prob) in enumerate(
            zip(results_with_scores, probs)):
        cumulative_prob += prob
        selected_results.append(result)

        if cumulative_prob >= p:
            print(
                f"[DEBUG] Selected {i+1} documents (cumulative_prob={cumulative_prob:.4f} >= p={p})")
            break
...
def relative_threshold_filter(results_with_scores: List[Tuple[Any, float]],
    ...
    return score_threshold_filter(
        results_with_scores, threshold, higher_better=True)


def elbow_method_filter(
        results_with_scores: List[Tuple[Any, float]]) -> List[Any]:
    """
...
def filter(rag_db, query: str, method: str = "top_p",
    ...
    if hasattr(rag_db, 'lookup_with_scores'):
        ...
        raise ValueError(
            f"Database {type(rag_db)} doesn't support score-based retrieval")
