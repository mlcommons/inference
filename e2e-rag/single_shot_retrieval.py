...
def _load_document_text(metadata, base_dir=None,
                        default_base_dir="doc_html", max_chars=FULL_DOC_MAX_CHARS):
...
def _convert_results_to_entries(
        results, limit=5, full_doc=False, base_dir=None, default_base_dir="doc_html"):
    entries = []
    seen_ids = set()
    count = 0
    for doc in results:
...
if __name__ == "__main__":
    ...
    # This includes: Common, General, BM25, Vector, Strategy, and Reranking
    # parameters
    add_all_args(args)
    ...
    if os.path.exists(db_file_path):
        ...
        if not args.ingest:
            raise ValueError(
                "Either --database (existing) or --ingest (to create new) must be provided")
        ...
        # This should be available after ingestion
        num_passages = len(rag_db._doc_list)
        toc = time.time()
        ingestion_speed = num_passages / (toc - tic)
        print(
            f"Ingestion of {num_passages} passages took {toc - tic:.2f} seconds. {ingestion_speed:.2f} docs/sec")
    ...
    if args.eval:
        ...
        # Build strategy_params with correct parameter names for filter
        # function
        ...
        metrics = run_evaluation(
            ...
            result_handler=handle_result if (
                args.generate_answer or args.save_results) else None,
            **strategy_params
        )
        ...
        results_data = {
            ...
            "accuracy": metrics.get('legacy_score', 0.0),
            "metrics": metrics
        }
        ...
        if args.generate_answer:
            ...
            answer_value = _generate_llm_answer(
                args.query, doc_entries, llm_config)
