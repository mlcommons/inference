...
def process_pdfs(input_dir, output_dir, json_file=None,
                 max_passage_length=512, passage_overlap=50, max_files=None):
    ...
    try:
        ...
        for pdf_file in pbar:
            ...
            if output_file.exists():
                pbar.set_postfix(
                    {"Status": f"Skipped {pdf_file.name[:30]}..."})
                continue
            ...
            if text is not None:
                ...
                try:
                    ...
                    pbar.set_postfix(
                        {"Status": f"Failed {pdf_file.name[:30]}... ({e})"})
                    failed += 1
            else:
                pbar.set_postfix(
...
def main():
    ...
    parser.add_argument(
        '--json-file',
        help='Output JSON file path for passages data (enables JSON creation)')
    parser.add_argument(
        '--max-files',
        type=int,
