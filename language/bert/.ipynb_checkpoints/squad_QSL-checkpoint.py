    ...
    def __init__(self, total_count_override=None, perf_count_override=None, cache_path='eval_features.pickle'):
        ...
        if os.path.exists(cache_path):
            ...
            with open(cache_path, 'rb') as cache_file:
                ...
                print ("eval_features is ")
                # print (eval_features)
                # Modifying input data to lower fp
                # print ('')
                ...
                #     print (eval_features[i].input_ids)
                #     print ('----------------------')
                #     print (eval_features[i].input_mask)
                #     print ('----------------------')
                #     print (eval_features[i].segment_ids)
                
                # print (eval_features[1].input_ids)
                # print (type (eval_features[1].input_ids[0]))
                # print (np.array (eval_features[1].input_ids).dtype)
                print ('----------------------')
                print ('----------------------')


                # eval_features = sorted(eval_features, key=lambda x: sum(x.input_mask))

                # for eval_feature in eval_features: 
                #     # print (eval_feature)
                #     # print (eval_feature.input_ids)
                #     eval_feature.input_ids = eval_feature.input_ids.astype(np.int32)
                #     eval_feature.input_ids = eval_feature.input_ids.astype(np.int32)
                #     eval_feature.input_ids = eval_feature.input_ids.astype(np.int32)
            ...
            eval_examples = read_squad_examples(input_file=dataset_file,
                is_training=False, version_2_with_negative=False)# ----------------------- ABLE to do preprocessing here 
