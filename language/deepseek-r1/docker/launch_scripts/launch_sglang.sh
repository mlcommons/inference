...
# SGLang Docker Launch Script
# Replicates the functionality of launch_docker.sh for SGLang backend

# Set default values (can be overridden by environment variables)
MOUNT_SELF_AS_LOADGEN=${MOUNT_SELF_AS_LOADGEN:-true}
IMAGE_NAME=${IMAGE_NAME:-mlperf-sglang}
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help)
            ...
            echo "Options:"
            echo "  --image-name NAME        Docker image name (default: mlperf-sglang)"
            echo "  --container-name NAME    Container name (default: sglang)"
...
echo "Launching SGLang Docker container..."
...
# Build the base image using the Dockerfile for SGLang
docker build -f "$SCRIPT_DIR/../Dockerfile.sglang" -t "$BASE_IMAGE" "$SCRIPT_DIR/../.."
