...
def main():
    ...
    data_object = Dataset(dataset_path, spm_path=args.spm_path)
    ...
    for pred in results:
        ...
        preds = np.frombuffer(bytes.fromhex(pred['data']), np.int64).astype(np.int32).tolist()
        preds_token_ids.append(preds)
        
    preds_token_ids = tf.constant(preds_token_ids)
    preds_decoded_text = data_object.vocabulary.tf_tokenizer.detokenize(preds_token_ids).numpy().tolist()
    preds_decoded_text = [s.decode("utf-8") for s in preds_decoded_text]
