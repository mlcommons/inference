...
from liteml.ailabs_qat.layers.liteml_layers import (
    LiteMLMatmul,
    LiteMLAdd,
    LiteMLMul,
    LlamaRMSNorm,
...
class LlamaRotaryEmbedding(nn.Module):
    ...
    def _set_cos_sin_cache(self, seq_len, device, dtype):
        ...
        emb = torch.cat(
            (freqs, -freqs), dim=-1
        )  # Multiplied right half by -1 instead of the negation in the rotate function
...
def rotate_half(x):
    ...
    return torch.cat(
        (x2, x1), dim=-1
    )  # Added negation in the _set_cos_sin_cache function
...
def apply_rotary_pos_emb(
    ...
    q_embed = add_q(
        mul_q_cos(q, cos), (rotate_half(mul_q_sin(q, sin)))
    )  # with quantization

    k_embed = add_k(
        mul_k_cos(k, cos), (rotate_half(mul_k_sin(k, sin)))
    )  # with quantization
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        attn_output = self.matmul_pv(
            attn_weights, value_states
        )  # in1 per-tensor, in2 per-token
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        if attention_mask is not None:
            ...
            combined_attention_mask = torch.clip(
                combined_attention_mask, torch.finfo(inputs_embeds.dtype).min
            )
