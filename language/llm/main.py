...
class Item:
    ...
    def __init__(self, query_id, content_id, caption, prompt = None):
...
class RunnerBase:
    ...
    def run_one_item(self, qitem: Item):
        ...
        processed_results = []
        try:
            results = self.model.predict(qitem.prompt)
            processed_results = self.post_process(results, qitem.content_id, qitem.prompt, self.result_dict)
    ...
    def enqueue(self, query_samples):
        ...
        if len(query_samples) < self.max_batchsize:
            prompts, labels = self.ds.get_samples(idx)
            self.run_one_item(Item(query_id, idx, prompts, labels))
        else:
            bs = self.max_batchsize
            for i in range(0, len(idx), bs):
                prompts, labels = self.ds.get_samples(idx[i:i+bs])
                self.run_one_item(Item(query_id[i:i+bs], idx[i:i+bs], prompts, labels))
...
class QueueRunner(RunnerBase):
    ...
    def enqueue(self, query_samples):
        ...
        if len(query_samples) < self.max_batchsize:
            prompts, labels = self.ds.get_samples(idx)
            self.tasks.put(Item(query_id, idx, prompts, labels))
        else:
            bs = self.max_batchsize
            for i in range(0, len(idx), bs):
                ie = i + bs
                prompts, labels = self.ds.get_samples(idx[i:ie])
                self.tasks.put(Item(query_id[i:ie], idx[i:ie], prompts, labels))
...
def main():
    ...
    # dataset to use
    dataset_class, post_proc, kwargs = SUPPORTED_DATASETS[args.dataset]
    ds = dataset_class(data_path=args.dataset_path,
                        name=args.dataset,
                        count=count,
    ...
    for i in range(5):
        prompts, labels = ds.get_samples([0])
        _ = backend.predict(prompts)
