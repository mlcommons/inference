...
# Modifications Copyright (C) 2013 Xilinx
# modified by Ji Liu
...
class Resnet34Backbone(object):
    ...
    def __init__(self, data_format='channels_first'):
        ...
        self._stage2_downsample = self.conv_bn_block(1, 128, 1, (2, 2), 'stage2_downsample')
        ...
        self._stage3_downsample = self.conv_bn_block(1, 256, 1, (1, 1), 'stage3_downsample')
    ...
    def forward(self, inputs, training=False):
        ...
        for layer in self._conv7_bn_block:
            inputs = forward_module(layer, inputs, training=training)
        feature_layers.append(inputs)
        for layer in self._conv8_bn_block:
            inputs = forward_module(layer, inputs, training=training)
        feature_layers.append(inputs)
        for layer in self._conv9_bn_block:
            inputs = forward_module(layer, inputs, training=training)
        feature_layers.append(inputs)
        for layer in self._conv10_bn_block:
            inputs = forward_module(layer, inputs, training=training)
        feature_layers.append(inputs)
        for layer in self._conv11_bn_block:
            inputs = forward_module(layer, inputs, training=training)
        feature_layers.append(inputs)
    ...
    def residual_block(self, filters, kernel_size, name, padding='same', is_pre_stride=False, reuse=None):    
        if not is_pre_stride:
            residual_blocks = self.conv_bn_block(2, filters, kernel_size, strides=(1, 1), name=name)
        else:
            residual_blocks = self.conv_bn_block(1, filters, kernel_size, strides=(2, 2), name=name+'_1')
            residual_blocks.extend(self.conv_bn_block(1, filters, kernel_size, strides=(1, 1), name=name+'_2'))
        return residual_blocks

    def conv_block(self, num_blocks, filters, kernel_size, strides, name, padding='same', reuse=None):
        with tf.variable_scope(name):
            ...
            for ind in range(1, num_blocks + 1):
                conv_blocks.append(tf.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides,
                                                    padding=padding, data_format=self._data_format, activation=tf.nn.relu, use_bias=True,
                                                    kernel_initializer=self._conv_initializer(),
                                                    bias_initializer=tf.zeros_initializer(),
                                                    name='{}_{}'.format(name, ind), _scope='{}_{}'.format(name, ind), _reuse=None))
            return conv_blocks

    def conv_bn_block(self, num_blocks, filters, kernel_size, strides, name, padding='same', reuse=None):
        with tf.variable_scope(name):
            conv_bn_blocks = []
            for ind in range(1, num_blocks + 1):
                conv_bn_blocks.append(tf.layers.Conv2D(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding,
                                                       data_format=self._data_format, activation=None, use_bias=True,
                                                       kernel_initializer=self._conv_initializer(),
                                                       bias_initializer=tf.zeros_initializer(),
                                                       name='{}_{}'.format(name, ind), _scope='{}_{}'.format(name, ind), _reuse=None))
...
def multibox_head(feature_layers, num_classes, num_anchors_depth_per_layer, data_format='channels_first', strides=(1, 1)):
