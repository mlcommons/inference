...
def main() -> None:
    ...
    input_file_path = os.path.join(
        args.in_memory_binary_criteo_path, f"day_23_sparse.npy"
    )
    ...
    output_file_path = os.path.join(
        args.output_path, f"day_23_sparse_multi_hot.npz"
    )
    np.savez(output_file_path, **multi_hot_ids_dict)
    if args.copy_labels_and_dense:
        for part in ["labels", "dense"]:
            source_path = os.path.join(
                args.in_memory_binary_criteo_path, f"day_23_{part}.npy"
            )
            output_path = os.path.join(args.output_path, f"day_23_{part}.npy")
