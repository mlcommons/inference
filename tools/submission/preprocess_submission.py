...
def clean_invalid_results(args, log_path, config, system_desc, system_json, model, mlperf_model, division, is_closed_or_network):
    # cleanup invalid results
                        for scenario in list_dir(log_path, system_desc, model):
                            ...
                            if perf_is_valid:
                                ...
                                if has_power:
                                    ranging_path = os.path.join(scenario_path, "performance", "ranging")
                                    try:
                                    ...
                                    if not power_is_valid:
                                        print(f"Power result is invalid for {model} {scenario} scenario in {division} division. Removing...")
                            ...
                            if not is_valid: #Remove the scenario result
                                if scenario in ["Offline", "MultiStream"] or division == "open":# they can be inferred
                                    ...
                                    if not args.nomove_failed_to_open:
                                        target_results_path = change_first_directory_to_open(model_results_path);
                                        ...
                                        shutil.copytree(model_results_path, target_results_path)
                                ...
                                else:#delete this result, move other scenario results to open
                                    shutil.rmtree(os.path.dirname(scenario_path)) #delete other scenario results too
                                    print(f"{scenario} scenario result is invalid for {model} in {division} division. Accuracy: {accuracy_is_valid}, Performance: {perf_is_valid}. Removing all dependent scenario results...")
...
def infer_scenario_results(args, config):
    ...
    for division in sorted(list_dir(".")): #process closed and network before open
                ...
                for system_desc in list_dir(log_path):
                    ...
                    for model in list_dir(log_path, system_desc):
                        ...
                        if directory == "results":
                            clean_invalid_results(args, log_path, config, system_desc, system_json, model, mlperf_model, division, is_closed_or_network)
