...
class SubmissionLogs:
    def __init__(self, performance_log, accuracy_log, accuracy_result,
                 accuracy_json, system_json, loader_data={}) -> None:
...
class Loader:
    ...
    def load_single_log(
            self, path, log_type: Literal["Performance", "Accuracy", "AccuracyResult", "AccuracyJSON", "Test", "System"]):
        log = None
                    ...
                    for benchmark in list_dir(system_path):
                        ...
                        for scenario in list_dir(benchmark_path):
                            ...
                            acc_path = self.acc_log_path.format(
                                ...
                                benchmark=benchmark,
                                scenario=scenario)
                            acc_result = self.acc_result_path.format(
                                division=division,
                                submitter=submitter,
                                system=system,
                                benchmark=benchmark,
                                scenario=scenario)
                            acc_json = self.acc_json_path.format(
                                division=division,
                                submitter=submitter,
                                system=system,
                                benchmark=benchmark,
