...
def check_performance_dir(
    ...
    if is_network_system:
        ...
        if NETWORK_MODE_REQUIRED_SUBSTRING_IN_SUT_NAME not in sut_name:
            log.error(
                f"{fname} invalid sut name for network mode. expecting the substring" +
                f" '{NETWORK_MODE_REQUIRED_SUBSTRING_IN_SUT_NAME}' got '{sut_name}'"
            )
            is_valid = False
...
def is_system_over_network(division, system_json, path):
    ...
    if not is_valid:
        log.error(
            f"{path} incorrect network mode (={is_network_system})" +
            f" for division '{division}'"
        )
