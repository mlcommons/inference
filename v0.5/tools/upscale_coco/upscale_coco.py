...
def parse_args():
    parser = ArgumentParser(description="Upscale COCO dataset")
    ...
    parser.add_argument('--size', default=[1400,1400], type=int, nargs='+',
...
def upscale_coco(indir, outdir, image_dir, annotate_file, size, fmt):
    ...
    for idx in coco.imgs:
        ...
        image = Image.open(os.path.join(image_in_path, img['file_name']))
        image = image.resize(size, resample=Image.BILINEAR)
        image.save(os.path.join(image_out_path, img['file_name'][0:-3]+fmt))
        # Image dimensions
        img['width'] = size[0]
        img['height'] = size[1]
        count = count + 1
