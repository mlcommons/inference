...
def eval(
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for data in tqdm(data_layer.data_iterator):
                ...
                if args.use_conv_mask:
                    t_log_probs_e, t_encoded_len_e = encoderdecoder(
        ...
        print("\n".join(["dnn latency {} : {} ".format(k, v)
                         for k, v in latencies_dnn.items()]))
        print("\n".join(["prep + dnn latency {} : {} ".format(k, v)
...
def main(args):
    ...
    encoderdecoder = JasperEncoderDecoder(
        jasper_model_definition=jasper_model_definition, feat_in=1024, num_classes=len(ctc_vocab))
    ...
    if args.ckpt is not None:
        ...
        audio_preprocessor.load_state_dict(
    ...
    if args.steps is None:
        ...
        print('Have {0} examples to eval on.'.format(
